...
class Mapping:
    ...
    def virtex7lambda(self, hits):
        hits['SL'] = hits.apply(lambda x: x['FPGA']*2 + int(x['TDC_CHANNEL'] > config.NCHANNELS), axis=1)
        hits['LAYER'] = hits['TDC_CHANNEL'].map(config.VIRTEX7_LAYER)
        hits['WIRE_NUM'] = hits['TDC_CHANNEL'].map(config.VIRTEX7_WIRE)
        
        hits['X_POSSHIFT'] = hits['LAYER'].apply(lambda x: config.posshift_x[int(x)-1])
        hits['Z_POS'] = hits['LAYER'].apply(lambda x: config.posshift_z[int(x)-1])
